#!/usr/bin/env python3
"""
MLLM-5.1 — Discrete Diffusion Micro Language Model
Single-file, zero-dependency, pure Python 3.10+

A toy diffusion LM that sculpts text out of noise.
Core idea: build a bidirectional n-gram "topology" from a corpus, then
iteratively denoise a sequence of [MASK] tokens. Each step scores every
masked position against its left & right contexts (distances 1..3), samples
under an annealed temperature, and re-masks the least-confident fills.
Text is carved out of noise — forward *and* backward.

Usage
-----
  python MLLM-5.1.py                          # chat (default)
  python MLLM-5.1.py chat --steps 30 --seed 42
  python MLLM-5.1.py generate "what is an atom" --steps 30 --seed 42 --show-steps
  python MLLM-5.1.py generate "hello" --corpus ./my.txt --extra-tokens 8 14 --no-color

Principles (preserved)
----------------------
  • bidirectional n-gram topology (left_counts / right_counts, n=1..3)
  • discrete diffusion: start fully masked, iteratively fill + re-mask
  • annealed sampling: high temperature early (explore), low late (sharpen)
  • confidence-guided remasking: weakest fills get another pass
  • pure stdlib, deterministic with --seed, works offline

No install needed. Just Python 3.10+. Optionally install for `mllm51` CLI
...
# ───────────────────────────────────────────────────────────── embedded corpus
BUILT_IN_CORPUS = """The architecture of the universe is written in the language of mathematics. 
...
class DiscreteDiffusionEngine:
    """Iteratively denoise masked sequences using n-gram statistics."""
    ...
    def candidate_distribution(self, seq: Sequence[str], idx: int) -> dict[str, float]:
        """Pure (no mutation) softmax over candidates for position *idx*."""
        base = 0.0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="MLLM-5.1",
        description="Discrete diffusion LM that sculpts text from [MASK] noise using bidirectional n-gram statistics.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            examples:
              python MLLM-5.1.py generate "what is an atom" --steps 30 --seed 42
              python MLLM-5.1.py generate "hello" --show-steps --extra-tokens 8 14
              python MLLM-5.1.py chat --steps 24
              python MLLM-5.1.py --corpus ./my.txt chat --seed 1

            tips:
              --steps is the effort knob: more steps = more refinement.
              --seed makes output reproducible.  --show-steps visualizes denoising.
              In chat, type [quit] / quit / exit to leave,  :help for commands.
        """),
    )
    p.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    p.add_argument("--corpus", type=Path, default=None, help="path to training corpus (default: embedded built-in)")
    ...
    def add_decoding_args(sub):
        sub.add_argument("--steps", type=int, default=30, help="diffusion steps — effort knob (default: 30)")
        sub.add_argument("--extra-tokens", type=int, nargs=2, default=(10, 18), metavar=("MIN", "MAX"),
                         help="range of tokens to generate beyond prompt (default: 10 18)")
        # allow global flags also after subcommand for convenience
    ...
    gen = subs.add_parser("generate", help="one-shot generation from a prompt")
    gen.add_argument("prompt", help="prompt text; its tokens are locked in place")
    gen.add_argument("--show-steps", action="store_true", help="render intermediate diffusion states")
    add_decoding_args(gen)

    chat = subs.add_parser("chat", help="interactive REPL (default if no subcommand)")
    add_decoding_args(chat)
    chat.add_argument("--show-steps", action="store_true", help=argparse.SUPPRESS)

    return p


def load_topology(corpus_path: Path | None, term: Term) -> BidirectionalTopology:
    t0 = time.time()
    ...
    print(term.paint("dim", f"· topology: {len(topo.vocab)} vocab · {topo.tokens} tokens · {topo.sentences} sentences · {dt:.2f}s"))
...
def _decode(engine: DiscreteDiffusionEngine, prompt_text: str, steps: int,
            extra_tokens: tuple[int, int], on_step) -> tuple[list[str], DenoiseResult]:
...
def cmd_generate(args: argparse.Namespace, engine: DiscreteDiffusionEngine, term: Term) -> int:
    on_step = make_step_printer(term) if getattr(args, "show_steps", False) else None
    prompt_words, result = _decode(engine, args.prompt, args.steps, tuple(args.extra_tokens), on_step)
    render_result(term, prompt_words, result)
    return 0


def cmd_chat(args: argparse.Namespace, engine: DiscreteDiffusionEngine, term: Term) -> int:
    print(term.paint("magenta", term.paint("bold", "\n💬 MLLM-5.1 READY — sculpt from noise. Type [quit] to exit, :help for commands.")))
    print(term.paint("dim", "  (diffusion: start from [MASK]s → iteratively denoise conditioned on your prompt)\n"))
    step_printer = make_step_printer(term) if getattr(args, "show_steps", False) else None
    # also allow inline :show pattern — we keep a persistent on_step toggler
    show = bool(getattr(args, "show_steps", False))
    while True:
        try:
            raw = input(term.paint("bold", "You > ")).strip()
        ...
        if low in {"[quit]", "quit", "exit", ":quit", ":q"}:
            break
        if low in {":help", "help", "?"}:
            print(term.paint("dim", "  commands: [quit]/exit  :help  :clear  :steps N  :seed N  :show on|off"))
            print(term.paint("dim", "  flags at launch: --steps N --seed N --extra-tokens MIN MAX --corpus PATH --show-steps --no-color -v"))
            continue
        ...
        if low.startswith(":seed"):
            try:
                s = int(low.split()[1]) if len(low.split()) > 1 else -1
        ...
        if low.startswith(":show"):
            try:
                v = low.split()[1].lower() if len(low.split()) > 1 else "toggle"
                if v in {"on", "true", "1"}:
                    show = True
                    ...
                    show = not show
                step_printer = make_step_printer(term) if show else None
                print(term.paint("dim", f"  show-steps → {'on' if show else 'off'}"))
            except Exception:
                show = not show
                step_printer = make_step_printer(term) if show else None
            continue
        ...
        prompt_words, result = _decode(engine, raw, int(args.steps), tuple(args.extra_tokens), step_printer)
        render_result(term, prompt_words, result)
        print()
    return 0
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # print banner for interactive terminals
    if term.enabled:
        print(term.paint("cyan", BANNER.strip("\n")))
        print(term.paint("dim", f"  v{__version__}  ·  diffusion LM  ·  topology n=3  ·  sculpt-from-noise"))
        print()
    else:
        print(f"MLLM-5.1 v{__version__} — diffusion LM")

    # validate extra-tokens
    try:
        extra = tuple(args.extra_tokens) if hasattr(args, "extra_tokens") else (10, 18)
    except Exception:
        extra = (10, 18)
    if isinstance(extra, (list, tuple)) and len(extra) == 2 and extra[0] > extra[1]:
        print("mllm51: error: --extra-tokens MIN must be <= MAX", file=sys.stderr)
        return 2

    corpus_path = Path(corpus_arg) if corpus_arg else None
    try:
        topo = load_topology(corpus_path, term)
    except (OSError, ValueError) as exc:
        print(f"mllm51: error: cannot load corpus {corpus_path or 'embedded'}: {exc}", file=sys.stderr)
        return 2

    # patch args for downstream
    args.extra_tokens = extra
    # ensure steps exists (default 30 for bare chat)
